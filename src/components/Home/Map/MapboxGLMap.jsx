...
export const MapboxGLMap = ({
    ...
    const ownershipStyles = ['Federal',
        ...
        'Other private',
        '#E9D841']
    const ownershipLegend = ownershipStyles.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);
    ...
    const vernalPoolsLegend = vernalPools.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);
    ...
    const conservedAreas = [
        'Conserved Area',
        '#713886'
    ]
    const conservedAreasLegend = conservedAreas.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);

    const newMarketTaxCredits = [
        'NMTC',
        '#0C1243'
    ]
    const newMarketTaxCreditsLegend = newMarketTaxCredits.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);

    const qualifiedOpportunityZones = [
        'QOZ',
        '#0C4315'
    ]
    const qualifiedOpportunityZonesLegend = qualifiedOpportunityZones.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);
    ...
    const railRoadsLegend = railRoads.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);
    ...
    const privateRoadsLegend = privateRoads.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);
    ...
    const maineMillsLegend = maineMills.reduce(function(result, value, index, array) {
        if (index % 2 === 0)
            result.push(array.slice(index, index + 2));
        return result;
    }, []);
    ...
    const initMap = () => {
        ...
        const mapboxGlMap = new mapboxgl.Map({
            ...
            bounds: [-71.562744,42.999217,-66.509033,47.547140]

        })

        mapboxGlMap.addControl(new mapboxgl.NavigationControl())
        mapboxGlMap.addControl(new mapboxgl.FullscreenControl())
        mapboxGlMap.addControl(new mapboxgl.AttributionControl({
            customAttribution: "<div class='smallViewport' <strong>Additional data available with a desktop site</strong> <br> </div>"
        }))
    ...
    useEffect(() => {
        ...
        if (!statefulMap) {
            ...
            if (getSelectedID()) {
                ...
                statefulMap.setPaintProperty('megaRegions-highlight', 'line-color', 'rgba(0,0,0,0)')
            }
    ...
    useEffect(() => {
        ...
        if (!statefulMap) {
            ...
            {
                if ((!(myForm.mySelectKey == null)) && (!(myForm.mySelectKey == 'Greenfield') && (!(myForm.mySelectKey == 'Brownfield')))) {
                    statefulMap.setFilter(
                        'siteSelection',
                        ['==', 'Site Type', `${myLabel.mySelectLabel}`]
                    );
                }

                else if (((myForm.mySelectKey == 'Greenfield') || (myForm.mySelectKey == 'Brownfield'))) {
                    statefulMap.setFilter(
                        'siteSelection',
                    );
                    statefulMap.setFilter(
                        'siteSelection',
                        ['==', `${myForm.mySelectKey}`, "Yes"]
                    );
            ...
            statefulMap.setLayoutProperty('megaRegions-solid-line', 'visibility', `${visibilityH}`)
            statefulMap.setPaintProperty(
            ...
            statefulMap.setLayoutProperty('megaRegions-solid-fill', 'visibility', `${visibilityI}`)
            statefulMap.setPaintProperty(
            ...
            statefulMap.setLayoutProperty('megaRegions-highlight', 'visibility', `${visibilityJ}`)
            statefulMap.setPaintProperty(
            ...
            statefulMap.setLayoutProperty('road-secondary-tertiary', 'visibility', `${visibilityL}`)
            statefulMap.setPaintProperty(
            ...
            statefulMap.setLayoutProperty('privateRoads', 'visibility', `${visibilityL}`)
            statefulMap.setPaintProperty(
                'privateRoads',
                'line-opacity',
                parseInt(`${sliderValueL}`, 10) / 100
            ...
            statefulMap.setLayoutProperty('maineMills', 'visibility', `${visibilityM}`)
            statefulMap.setPaintProperty(
                'maineMills',
                'icon-opacity',
    ...
    return (
        <div>
            {loadWidth &&
                <div className={"staticMap"} style={{
                backgroundImage: `url('https://api.mapbox.com/styles/v1/mapbox/outdoors-v11/static/[-71.562744,42.999217,-66.509033,47.547140]/${loadWidth}x${loadHeight}@2x?access_token=pk.eyJ1IjoiZGFuc3dpY2siLCJhIjoiY2l1dTUzcmgxMDJ0djJ0b2VhY2sxNXBiMyJ9.25Qs4HNEkHubd4_Awbd8Og')`,
                ...
                bottom: "0vh",
                position: "absolute",
                visibility: ((statefulMap === null) ?
                    "visible" : "hidden")
            }}>
            </div>}
            <div style={{width: "100vw", top: "0vh", left: "0", bottom: "0vh", position: "absolute"}}
                 ref={mapContainer}/>

            <div className={'legendBox'} style={{padding:
                `${visibilityA}`==="visible" ||
                `${visibilityC}`==="visible" ||
                `${visibilityD}`==="visible" ||
                `${visibilityE}`==="visible" ||
            ...
            <Menu customBurgerIcon={ <FiLayers /> } isOpen={ true } noOverlay={true}>
                ...
                >
                    ...
                    <Layer
                        ...
                        layerDescription={'Mapbox Satellite is a global tileset of high-resolution satellite imagery. The imagery data comes from a variety of commercial providers, as well as open data from NASA, USGS, and others. It’s color-corrected and blended together into a single raster tileset.'}
                        defaultChecked={false}
                        onToggleChange={ (e) => {
                            if(e.target.checked === false) {
                                setVisibilityB('none')
                ...
                >
                    ...
                    <Layer
                        ...
                        layerDescription={'Megaregions defined via county-wood basket groups.'}
                        defaultChecked={false}
                        onToggleChange={ (e) => {
                            if(e.target.checked === false) {
                                setVisibilityH('none')
                ...
                >
                    ...
                    <Layer
                        layerTitle={'Ports'}
                        sourceDetails={'Indufor and FOR/Maine 2020'}
                        layerDescription={'Major Wood Exporting and Importing Ports in Maine.'}
                        defaultChecked={false}
                        onToggleChange={ (e) => {
                            if(e.target.checked === false) {
                                setVisibilityN('none')
                ...
                >
                    ...
                    <Layer
                        layerTitle={'Rail'}
                        sourceDetails={'https://maine.hub.arcgis.com/datasets/27c9cef0de15475aa7461933e0294391'}
                        layerDescription={'Statewide railroad coverage for Maine at 1:24000 scale or better.'}
                        defaultChecked={false}
                        onToggleChange={ (e) => {
                            if(e.target.checked === false) {
                                setVisibilityK('none')
